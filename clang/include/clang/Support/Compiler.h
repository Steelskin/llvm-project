...
/// be imported by consumers (e.g. plugin registry storage whose definition may
/// live in the same object, a static library, or an import library). Using
/// CLANG_ABI there would force dllimport in consumers; using LLVM_ABI_EXPORT
/// would force dllexport even in a static clang build, leaking the symbol into
/// any DLL that embeds the clang static lib (libclang.dll).
