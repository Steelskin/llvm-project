...
function(llvm_update_pch name kind)
  ...
  # determines PCH compatibility (see the reuse branch below). This mirrors the
  # LLVM_EXPORTS / LLVM_BUILD_STATIC compile definitions applied elsewhere in
  # this file. When export annotations are disabled there is a single mode and
  ...
  # but is orthogonal to LLVM_ABI -- e.g. clang's CLANG_BUILD_STATIC /
  # CLANG_EXPORTS, which clang always applies on Windows regardless of LLVM's
  # export annotations. add_clang_library/add_clang_executable set
  # LLVM_PCH_ABI_TAG before creating the target so it is folded into the mode
  # here. A consumer in a different ABI dimension than its PCH provider (e.g. a
  # clang lib carrying CLANG_BUILD_STATIC reusing an LLVM provider's PCH that
  # does not) then takes the "different mode" branch below and builds its own
  # PCH, whose predefines match -- otherwise clang-cl warns -Wclang-cl-pch on
  # the macro-set mismatch even though the LLVM PCH content never uses CLANG_ABI.
  if(LLVM_PCH_ABI_TAG)
  ...
  if(ARG_PRECOMPILE_HEADERS)
    ...
    if(NOT ARG_DISABLE_PCH_REUSE)
      ...
      # Record the export mode this PCH was compiled in and the header(s) it
      # used, so a dependant in a different mode can build its own PCH instead
      # of reusing this one (see the reuse branch below).
